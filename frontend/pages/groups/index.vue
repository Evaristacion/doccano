    ...
    async createGroup() {
      try {
        const groupData = {
          ...
          permission_ids: this.createForm.selectedPermissions.map(p => p.id || p), // Extrair apenas os IDs
        };
        console.log("Creating group with data:", groupData); // Verificar os dados enviados
        ...
        alert("Error creating group: " + JSON.stringify(error.response?.data));
      }
    },
    async editGroup() {
      try {
        const groupData = {
          ...
          permission_ids: this.editForm.selectedPermissions.map(p => p.id || p), // Extrair apenas os IDs
        };
        console.log("Editing group with data:", groupData); // Verificar os dados enviados
        ...
        alert("Error editing group: " + JSON.stringify(error.response?.data || error.message));
      }
    },
    async deleteGroup(groupId) {
      if (confirm('Are you sure you want to delete this Group?')) {
        try {
          ...
          alert("Error deleting group: " + JSON.stringify(error.response?.data));
        }
      }
    }
  }
